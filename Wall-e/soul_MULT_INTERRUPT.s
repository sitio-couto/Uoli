@ NOTA SOBRE A READ_SONAR:
@Para evitar defasagem do tempo do sistema, a soul busca manter sempre as
@interrupcoes ativas salvando o SPSR na pilha do respectivo modo para evitar
@que uma interrupcao do mesmo modo sobrescreva o SPSR. O preco que se paga
@para manter o tempo uniforme e a sobrescrita de leituras do sonar feitas pelo
@usuario:  se houver uma interrupcao durante a leitura do sonar e o IRQ testar
@alguma proximity_callback, ele pode sobrescrever a leitura do sonar do usuario.
@Isso pode gerar alguma inconsistencia

.org 0x0
.set USER_CODE,     0x77812000  @ Define inicio do codigo do usuario
.set USER_SP,       0x77817004  @ Define incio da pilha do usuario
.set MAX_CALLBACKS, 8
.set MAX_ALARMS,    8
.section .iv,"a"
...
SYSTEM_TIME:  .word 0   @ Tempo de acesso exclusivo do sistema
IRQ_FLAG:     .word 0   @ Flag que indica se as callbacks ja estao sendo tratadas
USER_TIME:    .word 0   @ Tempo de acesso livre ao usuario

POSITION_BITS_CALLBACKS:  .word 0   @ Vetor de bits que indica posicoes ocupadas
POSITION_BITS_ALARM:		  .word 0   @ Vetor de bits que indica posicoes ocupadas

SONAR_CALLBACKS:            .skip 16  @ Sonares a serem verificados
DISTANCE_CALLBACKS:         .skip 32  @ Distancia minima de disparo da callback
ADDRESS_CALLBACKS:	        .skip 32  @ Funcao do usuario a ser chamada

ADDRESS_ALARM:		          .skip 32  @ Funcao do usuario a ser chamada
TIME_ALARM:                 .skip 32  @ Hora de disparo do alarme
...
.skip 16
IRQ_SPSR_SLOT:    .word 0   @ Guarda SPSR do do IRQ no caso de multiplas interrupcoes
...
IRQ_HANDLER:
    ...
    mrs r12, SPSR                 @ Salva o PCSR caso haja um novo IRQ durante o tratamento das callbacks
    push {r12}                    @ Guarda o CPSR do usuario iterropido em uma posicao reservada
...
continue_irq:
    msr CPSR_c, #0b11010010       @ desabilita interrupcao durante a recuperacao do CPSR do usuario
    pop {r12}                     @ Tira o SPSR da pilha
    msr SPSR, r12                 @ Restaura o SPSR original
...
SUPERVISOR_HANDLER:
    ...
    push {r4-r12, lr}
    mrs r12, SPSR             @ Copia SPSR antes de habilitar interrupcoes
    push {r12}                @ Poe o SPSR na pilha
    msr CPSR_c, #0b00010011   @ Habilita interrupcoes apos salvar os dados
    ...
    msr CPSR_c, #0b11010011 @ Desabilita interrupcoes durante a recuperacao dos dados
    pop {r12}               @ Pega o SPSR salvo na pilha
    msr SPSR, r12           @ Recupera o SPSR
    pop {r4-r12, lr}
...
get_time: @ ID = 20
      @ Parametros:
      @ R0 = endereco do user para guardar o valor
      mov r2, r0
      ldr r1, =USER_TIME
      ldr r0, [r1]
      str r0, [r2]
...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
@ call_user_code:
@ Essa funcao serve de intermediario para que o system possa invocar uma funcao
@ do usuario em modo USER e retornar para o modo super usuario (no caso IRQ)
