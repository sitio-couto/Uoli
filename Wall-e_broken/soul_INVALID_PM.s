...
IRQ_HANDLER:
    ...
    mrs r0, SPSR                  @ Salva o SPSR antes de habilitar interrupcoes
    push {r0}                     @ evitando perda de dados do estado anterior
    ...
    cmp r1, #2
    bhi end_irq     @ Se for um terceiro IRQ simultaneo, nao trata callbacks.

    b search_alarm_callbacks   @ Se for, no maximo, o segundo IRQ simultaneo, trata alarmes somente.
    return_alarm:              @ Esse rotulo evita o uso de lr (No caso de IRQ simultaneos, o lr
                               @ eh sobrescrito na hora da interrpcao sendo perdido, fica mais
                               @ simples evita-los em primeiro lugar)
    ldr r0, =IRQ_COUNTER
    ldr r1, [r0]
    cmp r1, #1
    bhi end_irq                   @ Se houver IRQs simultaneos, nao trata as callbacks de proximidade

    b search_proximity_callbacks  @ Se for o unico IRQ em andamento, trata as callbacks

end_irq:
    msr CPSR_c, #0b11010010     @ Desabilita interrupcoes durante a recuperacao do SPSR

    @ Saindo de um IRQ, decrementa e atualiza o IRQ_COUNTER
    ldr r0, =IRQ_COUNTER
    ldr r1, [r0]
    sub r1, r1, #1
    str r1, [r0]

    pop {r0}                    @ evitando que o SPSR seja sobrescrito no caso de um IRQ
    msr SPSR, r0                @ dentro de um IRQ.
...
search_proximity_callbacks:
    ...
    mov r11, #0                           @ indexa as posicoes dos vetores
...
loop_bit_check_proximity:         @ loop pelas posições do vetor
    ...
    @ Realiza uma syscall para a read_sonar evitando o uso de lr
    push {r1-r3}
    mov r7, #16
    svc 0x0
    pop {r1-r3}
    ...
    push {r1-r12}                     @ Salva dados do IRQ para executar a callback
    mov r1, #0
    msr CPSR_c, #0b00010000           @ Muda para o modo usuario
    b call_user_code                  @ Se menor, executa a callback
    return_from_proximity_user_code:  @ Permite o retono da callback
    pop {r1-r12}                      @ Recupera todos os dados

next_position_callback:            @ Organiza para selecionar a proxima posicao do vetor de bits
...
search_alarm_callbacks:
    ...
    ldr r3, =USER_TIME                   @ Carrega o endereco do tempo de usuario atual
...
next_position_alarm:               @ Organiza para a proxima posicao do vetor de bits
    mov r12, r12, lsr #1           @ Shifta mask para proxima posicao
    add r11, r11, #1               @ Incrementa index dos vetores
    cmp r12, #0                    @ Confere se passou todas as posicoes
...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
SUPERVISOR_HANDLER:
    msr CPSR_c, #0b00010011     @ Habilita interrupcoes durante as syscalls

    cmp r7, #0                  @ Syscall especial para retornar ao modo IRQ apos uma proximity_callback
    beq return_from_callback
    cmp r7, #1                  @ Syscall especial para retornar ao modo IRQ apos uma alarm_callback
...
@ do usuario e retornar para o modo super usuario exatamente de onde havia parado.
@ Alem de impedir que o usuario de um jump para o codigo do SO, essa funcao tambem
@ permite que o codigo do usuario seja executado em modo usuario evitando falhas
@ de seguranca.
call_user_code:
    push {r1, lr}   @ Preserva a identificacao da syscall de retorno e o lr do user
    blx r0          @ Pula para o codigo de usuario passado como parametro
    pop {r1, lr}    @ Recupera a identificacao da syscall de retorno e o lr do user

    mov r7, r1      @ Retorna ao SO com uma syscall especial identificada por R1
    svc 0x0
