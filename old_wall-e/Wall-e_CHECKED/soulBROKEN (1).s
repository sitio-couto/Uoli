...
search_proximity_callbacks:
    ...
    mov r12,  #1
    mov r12, r12, lsl #MAX_CALLBACKS      @ It
...
@
@    @ Carrega tempo do alarme
@    ldr r0, [r1, r11, lsl #2]     @ Pega o tempo de disparo do alarme
@    ldr r9, [r3]                  @ Pega o USER_TIME
@    cmp r0, r9                    @ Verifica se tempo foi alcancado
@    bhi next_position_alarm       @ Se maior, retorna sem tratar callback
@
@    @ Remove a callback do vetor de bits
@    bic r4, r4, r12                @ Apaga bit da posicao com a callback tratada
@    str r4, [r5]                   @ Atualiza o vetor de bits das posicoes
...
@    pop {r0-r12}
@    cmp r0,#0
@    beq return_first_alarm
@    @cmp r0, #1
@    @beq return_second_alarm

@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

SUPERVISOR_HANDLER:
    cmp r7, #0                  @ Syscall especial para retornar ao modo IRQ apos uma callback
    beq return_from_proximity_callback
    @cmp r7, #1
    @beq return_from_alarm_callback
...
delay_flag:
    ...
    ldr r5, [r4, #GPIO_PSR] @ Le o valor devolvido pelo sonar no PSR
                            @ldr r10, =VET_TEST_3
                            @str r5, [r10]
    ldr r7, =0xFFF
...
call_user_code:
@ R0 = endereco da funcao callback
@ R1 = syscall de retorno
    blx r0          @ Pula para o codigo de usuario passado como parametro

    mov r7, #0      @ Retorna ao SO com uma syscall especial
    svc 0x0
